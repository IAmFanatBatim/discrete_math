...
int getExceptionArrayByA(const int *a, const int a_len, const int *b, const int b_len, int *c, int *c_len) {
    *c_len = 0;
    int a_index = 0;
    int b_index = 0;
    while (a_index < a_len || a_index < a_len && b_index < b_len) {
        if (b_index >= b_len || a[a_index] < b[b_index]) {
            c[*c_len] = a[a_index];
            *c_len += 1;
            a_index++;
        } else {
...
int main() {
    ...
    getExceptionArrayByA(first_array, first_len, second_array, second_len, result_array, &result_len);
